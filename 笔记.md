...
按需打包: 只打包import引入组件的js/css <br>
引入依赖库：
```sh
npm install react-app-rewired customize-cra babel-plugin-import
```
定义加载配置的 js 模块：根目录下创建 config-overrides.js，添加如下代码：
```js
const {override, fixBabelImports} = require('customize-cra');

module.exports = override(
    // 针对 antd 实现按需打包： 根据import来打包（使用 babel-plugin-import 插件）
...
```
然后修改 package.json 文件中的代码为如下的代码：
```js
  "scripts": { 
...
！！！ 如果后续引入 less 文件报错时可以按照如下方法处理：
1、安装customize-cra-less-loader:
```sh
npm install customize-cra-less-loader
```
2、修改 config-override.js 文件配置
```js
const { override, fixBabelImports } = require('customize-cra')
...
## 6. 引入路由
    ...
    注册路由:
      <BrowserRouter>  
      <Routers>    // V6 版本 Routes 替换了 V5 的 Switch  只匹配其中一个
      <Route path='' component={}/>

<b>引入重置 css：<Br></b>

从github上下载 reset.css,放在public/css下，然后在 index.html中引入样式
